 ...
 * by the obsolete RFC 2396.
 *
 * <p>That specification has been superceded by RFCs 3986 and 3987 which defines
 * URIs and IRIs.
 * Both of these entities are now represented by the on class {@link IRI}
 * which is preferred over this one.
 *
...
public final class URI extends ResourceIdentifier
    implements Comparable<URI>, Serializable
{
    ...
    /**
     * Converts the given {@code ResourceIdentifier} into a {@code URI}.
     *
     * <p> If the given {@link ResourceIdentifier} {@code ri} is a
     * {@code URI}, returns {@code uri}. Otherwise, if it is an
     * {@code IRI}, obtains a new {@code URI} as if by calling
     * {@link IRI#toURI()}. Otherwise, returns a new {@code URI}
     * constructed as if by calling {@link #create(String)
     * URI.create(ri.toString())}. </p>
     *
     * @apiNote
     ...
     * of {@code URI} for further processing. </p>
     *
     * @param  ri  The {@code ResourceIdentifier} to be converted into
     *             a {@code URI}
     * @return A {@code URI} converted from the given
     ...
     *
     * @throws  IllegalArgumentException
     *          If the given {@code ResourceIdentifier} cannot be converted
     *          to a {@code URI}.
     */
    public static URI of(ResourceIdentifier ri) {
        try {
            return (ri instanceof URI) ? (URI)ri   // URI
                : (ri instanceof IRI) ? ((IRI)ri).toURI() // IRI
                : URI.create(ri.toString()); // throws NPE (ri == null);
        } catch (URISyntaxException x) {
            throw new IllegalArgumentException(x.getMessage(), x);
        }
    }
